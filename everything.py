...
leftTicksPerRad = 535.75
rightTicksPerRad = 116.46
...
def goForward(steps):
    ...
    fy = threading.Thread(target=forwardL, args= (.003,int(steps)))
    fx.start()
    fy.start()
    fx.join()
    fy.join()
...
def turnLeft(rad):
    ...
    fx = threading.Thread(target=forwardL, args= (timeMax,int(turnsLeft)))
    fy = threading.Thread(target=backwardsR, args= (timeRatio,int(turnsRight)))
    fx.start()
    fy.start()
    fx.join()
    fy.join()
...
def turnRight(rad):
    ...
    fx = threading.Thread(target=backwardsL, args= (timeMax,int(turnsLeft)))
    fy = threading.Thread(target=forwardR, args= (timeRatio,int(turnsRight)))
    fx.start()
    fy.start()
    fx.join()
    fy.join()
...
def readCSV():
    with open('./cvs_file_NEU') as csvfile:
